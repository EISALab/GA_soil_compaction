...
BoostedPerceptron <- function(DATA, IndVars, DepVar, Thresh, LastTrainDate, iterations, Cycles){
...
PrepBoostedPercep <- function(DATA, IndVars, DepVar, Thresh, LastTrainDate, iterations){
  TrainDATA <- DATA[DATA[,"Date"]<=LastTrainDate,]
  DepClasses = TrainDATA[,DepVar]>Thresh 
...
BoostedPerceptronSGD <- function(TrainDATA, IndVars, DepVar, DepClasses, LastTrainDate, Cycles, DayWeights){
  ...
  Weights     <- rep(1,length(IndVars)+1)
  for(n in 1:Cycles){
